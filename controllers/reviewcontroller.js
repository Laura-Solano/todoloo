...
const { Reviews, Reply } = require("../models");
...
router.put("/Edit/:reviewId", validateSession, (req, res) => {
  ...
  Reviews.update(editReview, query)
    .then((reviews) =>
      res.status(200).json({ reviews, message: "Your review has been updated" })
    )
    .catch((err) =>
      res.status(500).json({
        error: err,
        message: "Error occurred, updates could not be completed",
      })
    );
});
...
router.delete("/Delete/:reviewId", validateSession, (req, res) => {
  ...
  Reviews.destroy(query)
    ...
    .catch((err) =>
      res.status(500).json({
        error: err,
        message: "Error occurred, review has not been deleted",
      })
    );
});
...
router.get("/mine", validateSession, (req, res) => {
  ...
  user
    ...
    .catch((err) =>
      res.status(500).json({ error: err, message: "No reviews found" })
    );
});
